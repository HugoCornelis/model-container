...
#---------------------------------------------------------------------------
    ...
    def RegisterSolver(self, modelname, solver, solver_type):
        ...
        _modelname = "model_container::%s" % modelname

        solver_core = solver.GetCore()

        if solver_type == 'heccer':

            nmc_base.RegisterHeccerSolver(self._nmc_core, solver_core, ppist, _modelname)

        elif solver_type == 'chemesis3':

            nmc_base.RegisterChemesis3Solver(self._nmc_core, solver_core, ppist, _modelname)

        elif solver_type == 'des':

            nmc_base.RegisterDESSolver(self._nmc_core, solver_core, ppist, _modelname)
